        ...
        SwapClicked ->
            ...
            case prevModel.mode of
                ...
                _ ->
                    let
                        _ =
                            maybeDebugLog "swap button clicked, but it should be hidden right now!" ""
                    in
                    justModelUpdate
                        prevModel
        ...
        IntervalInputChanged input ->
            case prevModel.showIntervalModal of
                ...
                Nothing ->
                    let
                        _ =
                            maybeDebugLog "Interal input changed, but there is no interval modal open! Wut" ""
                    in
                    justModelUpdate prevModel
        ...
        IntervalUnitChanged newUnit ->
            case prevModel.showIntervalModal of
                ...
                Nothing ->
                    let
                        _ =
                            maybeDebugLog "Interal unit changed, but there is no interval modal open! Wut" ""
                    in
                    justModelUpdate prevModel
        ...
        ( newForeignCurrency, newDhTokenType ) =
            case ( initiatorRole prevModel.mode, ( prevModel.inputs.inType, prevModel.inputs.outType ) ) of
                ...
                _ ->
                    let
                        _ =
                            maybeDebugLog "unexpected currency types in input when trying to update model currency types" ""
                    in
...
subscriptions model =
    Sub.batch
        [ Time.every 2000000 (always Refresh)
